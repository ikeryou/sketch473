...
export class Item extends MyDisplay {
  ...
  private _getPathStr(isBg: boolean): string {
    ...
    if(isBg) {
      ...
      d += 'M ' + (p4.x) + ' ' + (p4.y) + ' '
      const offset = Util.map(Easing.instance.outExpo(this._rate.val), Util.map(this._noise, 0.1, 0.75, 0, 1), 0, 0.25, 1) * 2
  ...
  protected _update():void {
    ...
    if(this._isInner) {
      w = Util.map(Math.sin(this._c * 0.06), 1, 40, -1, 1)
      Tween.set(this.el, {
        width: w + '%',
        height: w + '%',
      })
